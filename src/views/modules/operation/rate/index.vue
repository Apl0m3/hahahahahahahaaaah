<template>
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      <el-form-item>
        <el-input v-model="dataForm.type"  clearable></el-input>
      </el-form-item>
      <el-form-item>
...
<!--      </el-table-column>-->
      <el-table-column
        ...
        header-align="center"
        align="center"
        :label="$t('manage.operation.rate.type')"
        >
        <template slot-scope="scope">
          <el-tag v-if="scope.row.type===1">{{$t('manage.operation.rate.type1')}}</el-tag>
          <el-tag v-if="scope.row.type===2">{{$t('manage.operation.rate.type2')}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        prop="rate"
        header-align="center"
        align="center"
        :label="$t('manage.operation.rate.rate')"
        >
        <template slot-scope="scope">
          <el-tag>{{scope.row.rate|doubleFilter}}%</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        prop="createAt"
        header-align="center"
        align="center"
        :label="$t('manage.operation.rate.createAt')"
        >
      </el-table-column>
      <el-table-column
        fixed="right"
...
<!--          <el-button type="text" v-if="isAuth('coupon:coupon:delete')" size="small" @click="deleteHandle(scope.row)">{{$t('manage.operation.rate.delete')}}</el-button>-->
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
